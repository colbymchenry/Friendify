...
function getInterestsHTML(prefix, interest_data) {
  for(var i in interest_data) {
     if (interest_data[i] instanceof Array) {
       interestHTML += '<ul>';
       interestHTML += getInterestElement(i, interestIndex, user_interests[(prefix + '_' + i).substring(1)] === 1);

       interestIndex++;

       interestHTML += '<ul>';
       for(var a in interest_data[i]) {
         interestHTML += getInterestElement(interest_data[i][a], interestIndex, user_interests[(prefix + '_' + i + '_' + interest_data[i][a]).substring(1)] === 1);
         interestHTML += '</li>';
         interestIndex++;
       }
      interestHTML += '</ul>';
...
function getInterestElement(id, name, checked) {
  return '\
      <li>\
        <div class="remember" style="text-align: left">\
            <div class="checkbox">\
                <label>\
                  <input type="checkbox" ' + (id === '' ? '' : 'id=' + id) + ' ' + (name === '' ? '' : 'name=' + name) + ' ' + (checked ? 'checked' : '') + ' onclick="setInterest(event);">' + id + '</input>\
                </label>\
            </div>\
        </div>\
  ';
